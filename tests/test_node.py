...
@pytest.mark.parametrize('klass', [
    LonelyRootNode, RootNode, InternalNode, LeafNode
])
...
def test_leaf_node_serialization_on_next_page():
    n1 = LeafNode(tree_conf, next_page=23)
    data = n1.dump()
...
def test_get_node_from_page_data():
    ...
    assert isinstance(
        Node.from_page_data(_tree_conf, data, 4),
        RootNode
    )
